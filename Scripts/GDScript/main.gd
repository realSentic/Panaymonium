...
var voiceline_text = [
	"They said this place has been closed for decades.",
	"I can see why."
]
...
func _on_voicelines_finished():
	...
	q.id = "reception_table"
	q.title = "Go to the reception table."
	q.description = "Find the missing fabric pieces and complete the tapestry."

	q.add_task("reception_area", "Touch the reception area")
	...
	objective_label.set_objective(q.title)

func _on_task_done(task_id: String):
	print("Task completed: ", task_id)

func _on_quest_done(quest: Quest):
	objective_label.play_complete_animation()
