...
func _process(delta: float) -> void:
	...
	for i: Control in position_offset_node.get_children():
		if wraparound_enabled:
			var max_index_range = max(1.0, (position_offset_node.get_child_count() - 1) / 2.0)
			...
			if i.get_index() > 0:
				var prev: Control = position_offset_node.get_child(i.get_index() - 1)
	...
	if wraparound_enabled:
		...
		var selected_child: Control = position_offset_node.get_child(selected_index)
		position_offset_node.position.x = lerp(
			position_offset_node.position.x,
			-(selected_child.position.x + selected_child.size.x / 2.0),
			smoothing_speed * delta
		)
